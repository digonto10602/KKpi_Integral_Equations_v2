...
def create_multiple_plots(input_file, N=10, output_prefix='plot'):
    ...
    for i, sigp_val in enumerate(sigp_values):
        ...
        # Plot realM vs sigk
        ax.plot(sigk_sorted, realM_sorted, 'o-', linewidth=2, markersize=6, 
                color='black', markerfacecolor='blue', markeredgecolor='black')

        # Labels and title
        ax.set_xlabel(r'$\sigma_k$', fontsize=18)
        ax.set_ylabel(r'Re($M$)', fontsize=18)
        ax.set_title(r'$\sigma_p = {:.4f}$'.format(sigp_val), fontsize=20)
