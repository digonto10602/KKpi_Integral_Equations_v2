...
def surface_plot_2(opefile, singularity_file, qvec_file, N):
    plt.rcParams.update({'font.size': 18})
    plt.rc('font',**{'family':'serif','serif':['Computer Modern Roman']})
    plt.rc('text', usetex=True)
    ...
    DATA1 = np.genfromtxt(opefile, skip_header=1, unpack=True)
    DATA2 = np.genfromtxt(singularity_file, unpack=True) 
    DATA3 = np.genfromtxt(qvec_file, unpack=True)
    ...
    #ax.plot(reqb2, imqb2, marker='o',markerfacecolor="yellow", markersize=10, color="black",linestyle='none', markeredgewidth=2, markeredgecolor='yellow')
    ax.plot(reqvec, imqvec, color="black",linestyle='solid', linewidth=2)
...
if len(sys.argv) > 1 and len(sys.argv)<5:
    ...
    N = int(sys.argv[4])
    opefile = filename 
    surface_plot_2(opefile, kern_sing_file, qvecfile, N) 
else: 
