...
def Gmat_surface(gmat_file, N):
    plt.rcParams.update({'font.size': 18})
    plt.rc('font',**{'family':'serif','serif':['Computer Modern Roman']})
    plt.rc('text', usetex=True)
    
    data = np.genfromtxt(gmat_file, unpack=True)
    ...
    Z1 = scipy.interpolate.griddata((i_ind, j_ind), re_gmat, (Xi, Yi), method='linear')
    Z2 = scipy.interpolate.griddata((i_ind, j_ind), im_gmat, (Xi, Yi), method='linear')
    ...
    h0 = ax[1].contourf(Xi, Yi, Z2, levels=np.linspace(low_limit, hi_limit, num=color_level))
    ...
    plt.tight_layout()
    outfilename = str(gmat_file).split(".")
...
def Gmat_surface_1(gmat_file, N):
    plt.rcParams.update({'font.size': 18})
    plt.rc('font',**{'family':'serif','serif':['Computer Modern Roman']})
    plt.rc('text', usetex=True)
    ...
    plt.tight_layout()
    outfilename = str(gmat_file).split(".")
    output = outfilename[0] + "_with_eps.pdf"
    #plt.savefig(output)
    plt.draw()
    plt.show()
    #plt.close()      
...
if len(sys.argv) > 1:
    ...
    N = int(sys.argv[2])
    gmat_file = filename 
    #Gmat_surface(gmat_file, N)
    Gmat_surface_1(gmat_file, N)
else:
